...
bucket* find_symbol_in_table(bucket * buckets, int capacity, key_object* key)
{
    ...
    while(1){
        ...
        if(bucket->key == NULL || strcmp(bucket->key->key_string, key->key_string) == 0)
        {
            return bucket;
        }
...
bool insert_symbol(symbol_table* table, symbol s)
{
    ...
    bool is_new_key;
    is_new_key = !actual_bucket->key;

    if (is_new_key)
        table->count ++;

    actual_bucket->key = malloc(sizeof(key));
    actual_bucket->data = malloc(sizeof(symbol));
